...
<body>
	...
	<script>
        function Clock() {    //Clock함수를 정의해 날짜와 시간을 업데이트
            ...
            let month = modifyNumber(dateinfo.getMonth() + 1);  //객체에서 월을 가져오고 1을 더해 월을 형식화 한 뒤 month에 저장함
			//왜 +1 ? getMonth의 반환은 0부터 시작하므로 +1을 해주어야함

            let day = modifyNumber(dateinfo.getDate());  //Date 객체에서 일을 가져와 day 변수에 저장
			//modifyNumber 함수는 한 자리 숫자에만 0을 추가하므로, 이미 두 자리 숫자인 경우에는 그대로 유지
			...
			setTimeout(Clock, 1000); //setTimeout으로 변경
		}
